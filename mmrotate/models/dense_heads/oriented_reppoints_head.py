...
class OrientedRepPointsHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 loss_bbox_init=dict(type='SmoothL1Loss', beta=1.0 / 25.0, loss_weight=0.5),
                 loss_bbox_refine=dict(type='SmoothL1Loss', beta=1.0 / 25.0, loss_weight=1.0),
    ...
    def offset_to_pts(self, center_list, pred_list):
        ...
        for i_lvl, _ in enumerate(self.point_strides):
            ...
            for i_img, _ in enumerate(center_list):
                pts_center = center_list[i_img][i_lvl][:, :2].repeat(1, self.num_points)    #pts_center: num_points cols
                ...
                pts = xy_pts_shift * self.point_strides[i_lvl] + pts_center     # pts : num_points cols corresponding to each RepPoint
                pts_lvl.append(pts)
    ...
    def get_adaptive_points_feature(self, features, pt_locations, stride):
        ...
        sampled_features = torch.zeros([pt_locations.shape[0], features.size(1), pt_locations.size(1), pt_locations.size(2)]).to(pt_locations.device)
    ...
    def pointsets_quality_assessment(self, pts_features, cls_score,
        ...
        qua = qua_cls + self.init_qua_weight * (qua_loc_init + qua_ori_init) + (1.0 - self.init_qua_weight) * (qua_loc_refine + qua_ori_refine) + qua_poc
        return qua,
    ...
    def dynamic_pointset_samples_selection(self,
        ...
        for gt_ind in range(num_gt):
            ...
            for level in range(num_level):
                ...
                #   TODO    topk(min(level_gt_mask.sum(), 6), largest=False) 控制范围
        ...
        for i in range(num_level):
            mask = (pos_inds_after_select >= inds_level_interval[i]) & (pos_inds_after_select < inds_level_interval[i + 1])
        ...
        pos_normalize_term = pos_level_mask_after_select * (self.point_base_scale * torch.as_tensor(self.point_strides).type_as(label)).reshape(-1, 1)
    ...
    def loss(self,
        ...
        pts_coordinate_preds_init_img = [item.reshape(-1, 2 * self.num_points) for item in pts_coordinate_preds_init_img]

        pts_coordinate_preds_refine_img = levels_to_images(pts_coordinate_preds_refine, flatten=True)
        pts_coordinate_preds_refine_img = [item.reshape(-1, 2 * self.num_points) for item in pts_coordinate_preds_refine_img]
        ...
        pts_preds_refine = torch.cat(pts_coordinate_preds_refine_img, 0).view(-1, pts_coordinate_preds_refine_img[0].size(-1))
